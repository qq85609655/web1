...
export class DataItemSearchComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  seeData() {
    if (this.treeNode.data.nodeType != 3) {
      this.tipWarnMessage('请选择数据子类节点后进行查询！');
      return;
    }
    this.dialogOpts.runSql2.visible = true;
    this.dialogOpts.runSql2.param.code = this.queryParam.code;
  }

  runOk() {
    this.dialogOpts.runSql2.visible = false;
    this.runItem(this.dialogOpts.runSql2.param.code);
  }
  ...
  runItem(code) {
    this.getHttpClient().post('datastandarditem/queryDataShow/' + code, null, null, (data) => {
      this.dialogOpts.runSql.visible = true;
      this.dialogOpts.runSql.data.datas = data.datas;//数据结果
      this.dialogOpts.runSql.data.itemDetailVos = data.itemDetailVos; // 表头
      this.dialogOpts.runSql.data.sqlName = data.sqlName; //对应表名
      this.dialogOpts.runSql.data.dataCount = data.dataCount; //表现在的数据总量
    });
  }
